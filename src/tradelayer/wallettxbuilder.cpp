        ...
        interfaces::Wallet* iWallet)
{
    ...
    int omniTxClass = TL_CLASS_C;
    if (!UseEncodingClassC(payload.size())) omniTxClass = TL_CLASS_B;
    ...
    switch(omniTxClass) {
        case TL_CLASS_B: { // declaring vars in a switch here so use an expicit code block
            ...
            if (!TradeLayer_Encode_ClassB(senderAddress,redeemingPubKey,payload,vecSend)) { return MP_ENCODING_ERROR; }
        break; }
        case TL_CLASS_C:
            if(!TradeLayer_Encode_ClassC(payload,vecSend)) { return MP_ENCODING_ERROR; }
        break;
    }
    ...
    if (!receiverAddress.empty()) {
        ...
        vecSend.push_back(std::make_pair(scriptPubKey, 0 < referenceAmount ? referenceAmount : TLGetDustThreshold(scriptPubKey)));
    }
        ...
        interfaces::Wallet* iWallet)
{
    ...
    // add payload output
    std::vector<std::pair<CScript, int64_t> > vecSend;
    if (!TradeLayer_Encode_ClassC(payload, vecSend)) {
        return MP_ENCODING_ERROR;
    }
    ...
    if (!receiverAddress.empty() && receiverAddress != feeAddress) {
        max_outputs = 3;
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(receiverAddress));
        vecSend.push_back(std::make_pair(scriptPubKey, TLGetDustThreshold(scriptPubKey)));
    }
