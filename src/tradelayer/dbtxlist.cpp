...
/** Returns a list of all Trade Layer transactions in the given block range. */
int CMPTxList::GetTLTxsInBlockRange(int blockFirst, int blockLast, std::set<uint256>& retTxs)
{
    int count = 0;
    leveldb::Iterator* it = NewIterator();
...
void CMPTxList::LoadAlerts(int blockHeight)
{
    ...
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        ...
        boost::split(vstr, itData, boost::is_any_of(":"), boost::token_compress_on);
        if (4 != vstr.size()) continue; // unexpected number of tokens
        if (atoi(vstr[2]) != TRADELAYER_MESSAGE_TYPE_ALERT || atoi(vstr[0]) != 1) continue; // not a valid alert
    ...
    for (std::vector<std::pair<int64_t, uint256> >::iterator it = loadOrder.begin(); it != loadOrder.end(); ++it) {
        ...
        if (TRADELAYER_MESSAGE_TYPE_ALERT != mp_obj.getType()) {
...
void CMPTxList::LoadActivations(int blockHeight)
{
    ...
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        ...
        boost::split(vstr, itData, boost::is_any_of(":"), boost::token_compress_on);
        if (4 != vstr.size()) continue; // unexpected number of tokens
        if (atoi(vstr[2]) != TRADELAYER_MESSAGE_TYPE_ACTIVATION || atoi(vstr[0]) != 1) continue; // we only care about valid activations
        uint256 txid = uint256S(it->key().ToString());
        loadOrder.push_back(std::make_pair(atoi(vstr[1]), txid));
    }
    ...
    for (std::vector<std::pair<int64_t, uint256> >::iterator it = loadOrder.begin(); it != loadOrder.end(); ++it) {
        ...
        if (TRADELAYER_MESSAGE_TYPE_ACTIVATION != mp_obj.getType()) {
    ...
    if (gArgs.IsArgSet("-tlactivationallowsender") || gArgs.IsArgSet("-tlactivationignoresender")) {
        AddAlert("tradelayer", ALERT_CLIENT_VERSION_EXPIRY, std::numeric_limits<uint32_t>::max(),
