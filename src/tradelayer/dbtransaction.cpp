...
CTLTransactionDB::CTLTransactionDB(const fs::path& path, bool fWipe)
{
    leveldb::Status status = Open(path, fWipe);
    PrintToConsole("Loading master transactions database: %s\n", status.ToString());
}

CTLTransactionDB::~CTLTransactionDB()
{
 ...
 */
std::vector<std::string> CTLTransactionDB::FetchTransactionDetails(const uint256& txid)
{
    assert(pdb);
    std::string strValue;
    ...
    if (status.ok()) {
        ...
        if (vStr.size() == 2) {
            ...
            PrintToLog("ERROR: Entry (%s) found in TLTXDB with unexpected number of attributes!\n", txid.GetHex());
        }
    } else {
        PrintToLog("ERROR: Entry (%s) could not be loaded from TLTXDB!\n", txid.GetHex());
    }
 ...
 */
void CTLTransactionDB::RecordTransaction(const uint256& txid, uint32_t posInBlock, int processingResult)
{
    assert(pdb);
 ...
 */
std::string CTLTransactionDB::FetchInvalidReason(const uint256& txid)
{
