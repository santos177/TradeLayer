...
std::vector<TransactionRestriction> CConsensusParams::GetRestrictions() const
{
    ...
    { //  transaction type                        version        allow 0   activation block
      //  ----------------------------------      -------------  -------   ------------------
        { TRADELAYER_MESSAGE_TYPE_ALERT,            0xFFFF,        true,     MSC_ALERT_BLOCK   },
        { TRADELAYER_MESSAGE_TYPE_ACTIVATION,       0xFFFF,        true,     MSC_ALERT_BLOCK   },
        { TRADELAYER_MESSAGE_TYPE_DEACTIVATION,     0xFFFF,        true,     MSC_ALERT_BLOCK   },
...
CMainConsensusParams::CMainConsensusParams()
{
    ...
    TL_FREEZE_WAIT_PERIOD = 4096; // ~4 weeks
    // Script related:
    PUBKEYHASH_BLOCK = 0;
...
bool ActivateFeature(uint16_t featureId, int activationBlock, uint32_t minClientVersion, int transactionBlock)
{
    ...
    bool supported = TRADELAYER_VERSION >= minClientVersion;
    switch (featureId) {
...
std::string GetFeatureName(uint16_t featureId)
{
    switch (featureId) {
        ...
        case FEATURE_FEES: return "Fee system (inc 0.05% fee from trades of non-Trade Layer pairs)";
