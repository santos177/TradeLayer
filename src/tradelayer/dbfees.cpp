...
CTLFeeCache::CTLFeeCache(const fs::path& path, bool fWipe)
{
    leveldb::Status status = Open(path, fWipe);
    PrintToConsole("Loading fee cache database: %s\n", status.ToString());
}

CTLFeeCache::~CTLFeeCache()
{
    if (msc_debug_fees) PrintToLog("CTLFeeCache closed\n");
}

// Returns the distribution threshold for a property
int64_t CTLFeeCache::GetDistributionThreshold(const uint32_t &propertyId)
{
    return distributionThresholds[propertyId];
}

// Sets the distribution thresholds to total tokens for a property / TL_FEE_THRESHOLD
void CTLFeeCache::UpdateDistributionThresholds(uint32_t propertyId)
{
    int64_t distributionThreshold = getTotalTokens(propertyId) / TL_FEE_THRESHOLD;
...
int64_t CTLFeeCache::GetCachedAmount(const uint32_t &propertyId)
{
    assert(pdb);
...
void CTLFeeCache::ClearCache(const uint32_t &propertyId, int block)
{
...
void CTLFeeCache::AddFee(const uint32_t &propertyId, int block, const int64_t &amount)
{
...
void CTLFeeCache::EvalCache(const uint32_t &propertyId, int block)
{
...
void CTLFeeCache::DistributeCache(const uint32_t &propertyId, int block)
{
    LOCK(cs_tally);
...
void CTLFeeCache::PruneCache(const uint32_t &propertyId, int block)
{
...
void CTLFeeCache::printStats()
{
    PrintToConsole("CTLFeeCache stats: nWritten= %d , nRead= %d\n", nWritten, nRead);
}

// Show Fee Cache DB records
void CTLFeeCache::printAll()
{
    int count = 0;
...
std::set<feeCacheItem> CTLFeeCache::GetCacheHistory(const uint32_t &propertyId)
{
    assert(pdb);
...
CTLFeeHistory::CTLFeeHistory(const fs::path& path, bool fWipe)
{
    leveldb::Status status = Open(path, fWipe);
    PrintToConsole("Loading fee history database: %s\n", status.ToString());
}

CTLFeeHistory::~CTLFeeHistory()
{
    if (msc_debug_fees) PrintToLog("CTLFeeHistory closed\n");
}

// Show Fee History DB statistics
void CTLFeeHistory::printStats()
{
    PrintToConsole("CTLFeeHistory stats: nWritten= %d , nRead= %d\n", nWritten, nRead);
}

// Show Fee History DB records
void CTLFeeHistory::printAll()
{
    int count = 0;
    leveldb::Iterator* it = NewIterator();
    for(it->SeekToFirst(); it->Valid(); it->Next()) {
...
int CTLFeeHistory::CountRecords()
{
...
void CTLFeeHistory::RollBackHistory(int block)
{
    assert(pdb);
...
std::set<int> CTLFeeHistory::GetDistributionsForProperty(const uint32_t &propertyId)
{
    assert(pdb);
...
bool CTLFeeHistory::GetDistributionData(int id, uint32_t *propertyId, int *block, int64_t *total)
{
    assert(pdb);
...
std::set<feeHistoryItem> CTLFeeHistory::GetFeeDistribution(int id)
{
    assert(pdb);
...
void CTLFeeHistory::RecordFeeDistribution(const uint32_t &propertyId, int block, int64_t total, std::set<feeHistoryItem> feeRecipients)
{
    assert(pdb);
