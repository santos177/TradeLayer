 ...
 */
bool TradeLayer_Encode_ClassB(const std::string& senderAddress, const CPubKey& redeemingPubKey,
        const std::vector<unsigned char>& vchPayload, std::vector<std::pair<CScript, int64_t> >& vecOutputs)
{
    ...
    while (nRemainingBytes > 0) {
        ...
        vecOutputs.push_back(std::make_pair(scriptMultisigOut, TLGetDustThreshold(scriptMultisigOut)));
    }

    // Add the Exodus marker output
    // CScript scriptExodusOutput = GetScriptForDestination(ExodusAddress());
    // vecOutputs.push_back(std::make_pair(scriptExodusOutput, TLGetDustThreshold(scriptExodusOutput)));
    return true;
}
 ...
 */
bool TradeLayer_Encode_ClassC(const std::vector<unsigned char>& vchPayload,
