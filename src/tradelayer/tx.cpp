...
std::string mastercore::strTransactionType(uint16_t txType)
{
    switch (txType) {
        ...
        case TRADELAYER_MESSAGE_TYPE_ALERT: return "ALERT";
        case TRADELAYER_MESSAGE_TYPE_DEACTIVATION: return "Feature Deactivation";
        case TRADELAYER_MESSAGE_TYPE_ACTIVATION: return "Feature Activation";
...
int CMPTransaction::logicMath_SendToOwners()
{
    ...
    uint32_t feeProperty = isTestEcosystemProperty(property) ? TL_PROPERTY_TMSC : TL_PROPERTY_MSC;
    ...
    if (version == MP_TX_PKT_V0) NotifyTotalTokensChanged(TL_PROPERTY_MSC, block); // fee was burned
...
int CMPTransaction::logicMath_SendAll()
{
    ...
    while (0 != (propertyId = ptally->next())) {
        ...
        if (ecosystem == TL_PROPERTY_MSC && isTestEcosystemProperty(propertyId)) {
            continue;
        }
        if (ecosystem == TL_PROPERTY_TMSC && isMainEcosystemProperty(propertyId)) {
            continue;
        }
...
int CMPTransaction::logicMath_TradeOffer()
{
    ...
    // Ensure only TL and TTL are allowed, when the DEx is not yet free
    if (!IsFeatureActivated(FEATURE_FREEDEX, block)) {
        if (TL_PROPERTY_TMSC != property && TL_PROPERTY_MSC != property) {
...
int CMPTransaction::logicMath_MetaDExTrade()
{
    ...
    if (!IsFeatureActivated(FEATURE_TRADEALLPAIRS, block)) {
        // Trading non-TL pairs is not allowed before trading all pairs is activated
        if ((property != TL_PROPERTY_MSC) && (desired_property != TL_PROPERTY_MSC) &&
            (property != TL_PROPERTY_TMSC) && (desired_property != TL_PROPERTY_TMSC)) {
...
int CMPTransaction::logicMath_MetaDExCancelEcosystem()
{
    ...
    if (TL_PROPERTY_MSC != ecosystem && TL_PROPERTY_TMSC != ecosystem) {
...
int CMPTransaction::logicMath_EnableFreezing()
{
    ...
    if (!IsFeatureActivated(FEATURE_FREEZENOTICE, block)) {
        ...
        liveBlock = params.TL_FREEZE_WAIT_PERIOD + block;
    }
...
int CMPTransaction::logicMath_Alert()
{
    ...
    if (alert_type == ALERT_CLIENT_VERSION_EXPIRY && TRADELAYER_VERSION < alert_expiry) {
