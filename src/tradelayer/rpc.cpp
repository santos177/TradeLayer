...
void PopulateFailure(int error)
{
    switch (error) {
        ...
        case MP_TX_IS_NOT_TL_PROTOCOL:
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No Trade Layer Protocol transaction");
...
static UniValue tl_getfeeshare(const JSONRPCRequest& request)
{
    ...
    if (ecosystem == 1) {
        receiversSet = STO_GetReceivers("FEEDISTRIBUTION", TL_PROPERTY_MSC, COIN);
    } else {
        receiversSet = STO_GetReceivers("FEEDISTRIBUTION", TL_PROPERTY_TMSC, COIN);
    }
...
static UniValue tl_getpayload(const JSONRPCRequest& request)
{
    ...
    if (parseRC < 0) PopulateFailure(MP_TX_IS_NOT_TL_PROTOCOL);
...
static UniValue tl_listblockstransactions(const JSONRPCRequest& request)
{
    ...
    LOCK(cs_tally);
    {
        pDbTransactionList->GetTLTxsInBlockRange(blockFirst, blockLast, txs);
    }
