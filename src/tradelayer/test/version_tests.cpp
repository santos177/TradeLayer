...
BOOST_FIXTURE_TEST_SUITE(tradelayer_version_tests, BasicTestingSetup)

BOOST_AUTO_TEST_CASE(version_comparison)
{
    BOOST_CHECK(TRADELAYER_VERSION > 1000000); // Omni Core v0.0.10.0
}
...
{
    BOOST_CHECK_EQUAL(TradeLayerVersion(), "0.7.0");
}

BOOST_AUTO_TEST_CASE(version_number)
{
    BOOST_CHECK_EQUAL(TRADELAYER_VERSION, 70000000);
}
