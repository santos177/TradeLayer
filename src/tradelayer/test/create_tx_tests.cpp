...
BOOST_FIXTURE_TEST_SUITE(tradelayer_create_tx_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(tltxbuilder_empty)
{
    TLTxBuilder builder;
    CMutableTransaction tx = builder.build();
...
BOOST_AUTO_TEST_CASE(tltxbuilder_from_existing)
{
    ...
    CMutableTransaction tx = TLTxBuilder(txBasis).build();
    BOOST_CHECK_EQUAL(rawTx, EncodeHexTx(CTransaction(tx)));
}

BOOST_AUTO_TEST_CASE(tltxbuilder_op_return)
{
    ...
    CMutableTransaction tx = TLTxBuilder()
        .addInputs(prevTxs)
        .build();
