...
BOOST_FIXTURE_TEST_SUITE(tradelayer_encoding_c_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(class_c_marker)
{
    ...
    BOOST_CHECK(TradeLayer_Encode_ClassC(vchPayload, vecOutputs));
...
BOOST_AUTO_TEST_CASE(class_c_with_empty_payload)
{
    ...
    std::vector<std::pair<CScript, int64_t> > vecOutputs;
    BOOST_CHECK(!TradeLayer_Encode_ClassC(vchEmptyPayload, vecOutputs));
