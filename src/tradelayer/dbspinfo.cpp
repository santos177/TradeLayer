...
CMPSPInfo::CMPSPInfo(const fs::path& path, bool fWipe)
{
    ...
    // implied_tl.issuer = EncodeDestination(ExodusAddress());
    implied_tl.updateIssuer(0, 0, implied_tl.issuer);
    implied_tl.prop_type = MSC_PROPERTY_TYPE_DIVISIBLE;
    implied_tl.num_tokens = 700000;
    implied_tl.category = "N/A";
    implied_tl.subcategory = "N/A";
    implied_tl.name = "Omni tokens";
    implied_tl.url = "http://www.omnilayer.org";
    implied_tl.data = "Omni tokens serve as the binding between Bitcoin, smart properties and contracts created on the Omni Layer.";

    // implied_tomni.issuer = EncodeDestination(ExodusAddress());
    // implied_ttl.updateIssuer(0, 0, implied_ttl.issuer);
    // implied_ttl.prop_type = MSC_PROPERTY_TYPE_DIVISIBLE;
    // implied_ttl.num_tokens = 700000;
    // implied_ttl.category = "N/A";
    // implied_ttl.subcategory = "N/A";
    // implied_ttl.name = "Test Omni tokens";
    // implied_ttl.url = "http://www.omnilayer.org";
    // implied_ttl.data = "Test Omni tokens serve as the binding between Bitcoin, smart properties and contracts created on the Omni Layer.";
...
bool CMPSPInfo::updateSP(uint32_t propertyId, const Entry& info)
{
    ...
    if (TL_PROPERTY_MSC == propertyId || TL_PROPERTY_TMSC == propertyId) {
        return false;
    }
...
bool CMPSPInfo::getSP(uint32_t propertyId, Entry& info) const
{
    ...
    if (TL_PROPERTY_MSC == propertyId) {
        info = implied_tl;
        return true;
    } else if (TL_PROPERTY_TMSC == propertyId) {
        // info = implied_tomni;
        return true;
    }
...
void CMPSPInfo::printAll() const
{
    ...
    for (uint32_t idx = TL_PROPERTY_MSC; idx <= TL_PROPERTY_TMSC; idx++) {
        Entry info;
